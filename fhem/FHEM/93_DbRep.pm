...
sub DbRep_Main {
 ...
 if ($opt eq "sumValue") {
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_deldoublets", $params, "DbRep_deldoubl_Done", $to, "DbRep_ParseAborted", $hash);  
 } 
...
sub DbRep_syncStandby {
 ...
 $irows      = 0;                                                            # DB-Abfrage zeilenweise für jeden Array-Eintrag
 $irowdone   = 0;
