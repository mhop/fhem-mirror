...
sub DbRep_Main {
 ...
 if ($opt eq "sumValue") {
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_sumval_DoParse", $params, "DbRep_sumval_ParseDone", $to, "DbRep_ParseAborted", $hash);
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("expfile_DoParse", $params, "expfile_ParseDone", $to, "DbRep_ParseAborted", $hash); 
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("impfile_Push", $params, "impfile_PushDone", $to, "DbRep_ParseAborted", $hash);
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_maxval_DoParse", $params, "DbRep_maxval_ParseDone", $to, "DbRep_ParseAborted", $hash);      
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_minval_DoParse", $params, "DbRep_minval_ParseDone", $to, "DbRep_ParseAborted", $hash);        
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_del_DoParse", $params, "DbRep_del_ParseDone", $to, "DbRep_ParseAborted", $hash);
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_currentfillup", $params, "DbRep_currentfillup_Done", $to, "DbRep_ParseAborted", $hash);
 } 
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_diffval_DoParse", $params, "DbRep_diffval_ParseDone", $to, "DbRep_ParseAborted", $hash);         
 } 
 elsif ($opt eq "insert") { 
      $params = {
         hash  => $hash,
     ...
     $hash->{HELPER}{RUNNING_PID} = BlockingCall("DbRep_insert", $params, "DbRep_insert_Done", $to, "DbRep_ParseAborted", $hash);           
 } 
 elsif ($opt eq "deviceRename") { 
     $params = {
         hash    => $hash,
...
sub delseqdoubl_ParseDone {
  ...
  my $rnam = $opt =~ /adviceRemain/ ? "number_rows_to_remain" : 
             $opt =~ /adviceDelete/ ? "number_rows_to_delete" :
             "number_rows_deleted"; 
...
sub sqlCmd_DoParse {
  ...
  if ($vars) {
      ...
      for my $pm (@pms) {
          ...
          eval { $dbh->do($pm) } or do { $err = encode_base64($@, "");
                                         Log3 ($name, 2, "DbRep $name - ERROR - $@");
                                         $dbh->disconnect;
...
sub sqlCmd_ParseDone {
  ...
  my ($erread, $state) = DbRep_afterproc ($hash, "sqlCmd");                     # Befehl nach Procedure ausführen
                        ...
                        eval {
                            ...
                            for my $delRow (@dayRows) {
                                if($day != 00 || $delRow->[0] !~ /$lastHour/) {
                                    ...
                                    Log3 ($name, 5, "DbRep $name - DELETE FROM history WHERE (DEVICE=$delRow->[1]) AND (READING=$delRow->[3]) AND (TIMESTAMP=$delRow->[0]) AND (VALUE=$delRow->[4])");
...
sub DbRep_prepareOnly {
  ...
  Log3 ($name, 4, "DbRep $name - SQL prepare: $sql");
...
sub DbRep_dbhDo {
  ...
  Log3 ($name, 4, "DbRep $name - simple do statement: $sql");
...
sub DbRep_delread {
 ...
 if($shutdown) {
     my $do = 0;
     foreach my $key(@allrds) {                                                   # Highlighted Readings löschen und save statefile wegen Inkompatibilitär beim Restart
         if($key =~ /<html><span/) {
             $do = 1;
 ...
 if(@rdpfdel) {
     ...
     foreach my $key(@allrds) {                                                  # delete($defs{$name}{READINGS}{$key}) if($key ne "state");
