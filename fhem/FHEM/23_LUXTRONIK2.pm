...
LUXTRONIK2_Set($$@)
{
  ...
  if ( $cmd =~ /^(synchronizeClockHeatPump|hotWaterTemperatureTarget|opModeHotWater)$/i ) 
   ...
   if ($cmd eq 'synchronizeClockHeatPump') {
      ...
      Log3 $name, 3, "set $name $cmd $val";
      return "$name Error: Wrong parameter given for hotWaterCircPumpDeaerate, use on,off"
         if $val !~ /on|off/;
      $hash->{LOCAL} = 1;
      $resultStr = LUXTRONIK2_SetParameter ($hash, $cmd, $val);
      if ($val eq "on" ) {    
      ...
      # only send if no Deaerate checkbox is selected at all.   
      elsif( ReadingsVal( $name, "hotWaterCircPumpDeaerate", "off" ) eq "off" ) { 
        $resultStr .= LUXTRONIK2_SetParameter ($hash, "runDeaerate", 0);   
      } 
      $hash->{LOCAL} = 0;
      return $resultStr;
   }
   elsif( int(@_)==4 && $cmd eq 'heatingSystemCircPumpDeaerate' ) { # Einstellung->Entlüftung
      Log3 $name, 3, "set $name $cmd $val";
      return "$name Error: Wrong parameter given for heatingSystemCircPumpDeaerate, use on,off"
         if $val !~ /on|off/;
      $hash->{LOCAL} = 1;
      $resultStr = LUXTRONIK2_SetParameter ($hash, $cmd, $val);
      if ($val eq "on" ) {    
...
<ul>
  ...
  <b>Set</b>
   <ul>A firmware check assures before each set operation that a heat pump with untested firmware is not damaged accidentally.
      ...
      <li><code>heatingSytemCircPumpDeaerate &lt;on | off&gt;</code><br>
