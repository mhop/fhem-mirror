...
# eval "use XML::Simple;1"    or $missingXML   .= "XML::Simple ";     # should be in standard Fhem installation
...
sub FRITZBOX_Readout_API_Check($);
...
sub FRITZBOX_Set_dectRing_Phone($);
...
sub FRITZBOX_Set_lock_filter_Profile($);
...
#get Fritzbox tr064command X_AVM-DE_OnTel:1             x_contact        GetInfo
#get Fritzbox tr064command X_AVM-DE_Dect:1              x_dect           GetInfo
#get Fritzbox tr064command X_AVM-DE_AppSetup:1          x_appsetup       GetInfo
#get Fritzbox tr064command X_AVM-DE_Homeauto:1          x_homeauto       GetInfo
...
sub FRITZBOX_Attr($@)
{
   ...
   my $hash = $defs{$name};
   my $avmModel = InternalVal($name, "MODEL", undef);
...
#         }
   ...
   if ($aName eq "enableCPUInfo") {
     ...
     if ($cmd eq "del" || $aVal == 0) {
       foreach (keys %{ $hash->{READINGS} }) {
         readingsDelete($hash, $_) if $_ =~ /^box_cpu.*?/ && defined $hash->{READINGS}{$_}{VAL};
       }
     }
   }
   ...
   if ($aName eq "enableDocsisInfo") {
     ...
     if ($cmd eq "del" || $aVal == 0) {
       foreach (keys %{ $hash->{READINGS} }) {
         readingsDelete($hash, $_) if $_ =~ /^docsis.*?/ && defined $hash->{READINGS}{$_}{VAL};
       }
     }
   }
   ...
   if ($aName eq "enableMobileInfo") {
     ...
     if ($cmd eq "del" || $aVal == 0) {
       foreach (keys %{ $hash->{READINGS} }) {
         readingsDelete($hash, $_) if $_ =~ /^mobileInfo[(\d+)_.*|_.*]/ && defined $hash->{READINGS}{$_}{VAL};
       }
     }
   ...
   readingsDelete($hash, "box_wan_AccessType") if defined $hash->{READINGS}{box_wan_AccessType}{VAL};
   readingsDelete($hash, "box_DNS_over_TLS") if defined $hash->{READINGS}{box_DNS_over_TLS}{VAL};
...
sub FRITZBOX_Set($$@)
{
   ...
   my $list =  " checkAPIs:" . ($hash->{APICHECKED} <= 0 ? "full" : "basic,full");
      $list .=  " password"
            .  " update:noArg"
            .  " inActive:on,off";
   ...
   if ($hash->{WEBCONNECT}) {
     ...
     if ( lc $cmd eq 'smarthome') {
       ...
       if (int @val == 2) {
         ...
         if ($val[1] =~ /^(.*?):(.*?)$/ ) {
           ...
           if($hash->{fhem}{fwVersion} < 820) {
              unless(    ($action =~ /tempOffset/ && $actionVal =~ /^-?\d+(\.[05])?$/ )
                      ...
                      || ($action =~ /boost/      && $actionVal >= 0 && $actionVal <= 120 && $actionVal =~ /\d{1,3}/)
         ...
         # set FritzBox smartHome 18 minTemp:7.5
         # return "Testausgabe1: $val[0] - $val[1]";

         $preDefDevice = $val[0];

         my $smartCat  = ReadingsVal($name, "shdevice" .$preDefDevice. "_category", "other");
         if($smartCat ne "THERMOSTAT") {
           $retMsg = "ERROR: first parameter: $preDefDevice not a valid reference for a SmartHome THERMOSTAT\n";
           return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet) ;
         }

         my $smartAIN  = ReadingsVal($name, "shdevice" .$preDefDevice. "_uid", undef);

         if(!defined($smartAIN)) {
           $retMsg = "ERROR: first parameter: $preDefDevice not a valid reference for a SmartHome AIN\n";
           return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet) ;
         }
         $smartAIN =~ s/ //g;

         my $smartName = ReadingsVal($name, "shdevice" .$preDefDevice, undef);
         ...
         if (      $action =~ /tempOffset/ && $actionVal =~ /^(-?\d+(\.\d+)?)$/ ) {
         ...
         } elsif ( $action =~ /switch/     && $actionVal =~ /^switch:([01])$/ ) {
           $newValue = "ON"  if $1 == 1;
           $newValue = "OFF" if $1 == 0;
         ...
         } elsif ( $action =~ /preDefSave/ && $actionVal =~ /^([-\w]+)$/ ) {
           $preDefName = $1;
           ...
           $preDefName   = $2;
           $preDefWeb    = $3 if $3;
         } elsif ( $action =~ /boost/      && $actionVal >= 0 && $actionVal <= 120 && $actionVal =~ /(\d{1,3})/ ) {
           my $timeAdd = $1 * 60;
           my $endtimestamp = time() + $timeAdd;
           my $luaPara = 'webservices/homeautoswitch.lua?ain=' .$smartAIN. '&switchcmd=sethkrboost&endtimestamp=' . $endtimestamp;

           FRITZBOX_Log $hash, 3, "para luaFunction: " . $luaPara;
           my $returnData = FRITZBOX_call_Lua_Query( $hash, $luaPara, "", "luaCall") ;

           # return Dumper $returnData;
       ...
       if ($action =~ /preDefLoad/ ) {
         ...
         if ($val[0] ne $preDefDevice) {
           ...
           FRITZBOX_Log $hash, 5, "SmartHome Device preDefLoad-> \n" . FRITZBOX_Helper_Dumper($hash, $retDataVgl, 5);
           ...
           if ($preDefWeb ne "A") {
             ...
             if (defined $result->{data}->{apply}) {
               if ($result->{data}->{apply} eq "ok") {
                 ...
                 FRITZBOX_Log $hash, 2, "SmartHome Device " . $val[0] . " - " . FRITZBOX_Helper_Dumper($hash, $result, 2);
         ...
         $retMsg = "ERROR: Unexpected result: " . FRITZBOX_Helper_Dumper($hash, $result);
         return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet);
         ...
         FRITZBOX_Log $hash, 5, "SmartHome Device preDefSave-> \n" . FRITZBOX_Helper_Dumper($hash, $returnData, 5);
         ...
         FRITZBOX_Log $hash, 5, "SmartHome Device preDefDel-> \n" . FRITZBOX_Helper_Dumper($hash, $returnData, 5);
         ...
         if(defined $result->{data}->{done}) {
           if ($result->{data}->{done}) {
             ...
             if ($action =~ /switch/) {
               readingsSingleUpdate($hash, "shdevice" . $val[0] . "_state", $newValue, 1);
             } elsif ($action =~ /tmpAdjust/) {
               readingsSingleUpdate($hash, "shdevice" . $val[0] . "_targetTemp", $newValue, 1);
             } elsif ($action =~ /tmpPerm/) {
               readingsSingleUpdate($hash, "shdevice" . $val[0] . "_tempOffset", $newValue, 1);
             } else {
             }
         ...
         $returnData = FRITZBOX_Get_SmartHome_Devices_List($hash, $val[0]);
   ...
   #          $retMsg = "Valid date/time";
       ...
       if (int @val == 1 && $val[0] =~ /^(on|off)$/) {
         ...
         if ($hash->{LUADATA}==1) {
           ...
           FRITZBOX_Helper_retMsg($hash, "ERROR: Unexpected result: " . FRITZBOX_Helper_Dumper($hash, $result), $retMsgbySet);
       ...
       return FRITZBOX_Helper_retMsg($hash, "ERROR: Unexpected result: " . FRITZBOX_Helper_Dumper($hash, $result), $retMsgbySet);
       ...
       if($val[0] =~ /notifyoff/) {
         ...
         if ($hash->{fhem}{fwVersion} >= 800) {
           ...
           FRITZBOX_Log $hash, 5, "ledsetting " . $val[0] . " - \n" . FRITZBOX_Helper_Dumper($hash, $result, 5);
       ...
       if ($result->{data}->{apply} ne "ok") {
         $retMsg = "ERROR: ledsetting " . $arg . " - " . FRITZBOX_Helper_Dumper($hash, $result);
         return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet);
       }
       ...
       if ($pIDs &&  $uniqueID !~ /[$pIDs]/) {
         ...
         if ($tr064Result[0]->{Error}) {
           ...
           if ($tr064Result[0]->{GetPhonebookListResponse}) {
             ...
             $retMsg = "ERROR: no phonebook ID's via TR-064:" . FRITZBOX_Helper_Dumper($hash, \@tr064Result);
             return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet);
           }
         }
       }
         ...
         if ($tr064Result[0]->{Error}) {
           ...
           if ($tr064Result[0]->{SetPhonebookEntryUIDResponse}) {
             if (defined $tr064Result[0]->{SetPhonebookEntryUIDResponse}->{NewPhonebookEntryUniqueID}) {
               ...
               $retMsg = "ERROR: no NewPhonebookEntryUniqueID via TR-064:" . FRITZBOX_Helper_Dumper($hash, \@tr064Result);
               return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet);
             }
           } else {
             $retMsg = "ERROR: no SetPhonebookEntryUIDResponse via TR-064:" . FRITZBOX_Helper_Dumper($hash, \@tr064Result);
             return FRITZBOX_Helper_retMsg($hash, $retMsg, $retMsgbySet);
           }
         }
       } elsif ($val[0] eq "del") {
...
#              FRITZBOX_call_TR064_Cmd ($hash, 0, \@tr064CmdArray);
     ...
     # dectRing
       ...
       if($id == -1 || ReadingsNum($name, "dect" .$id. "_ID", -1) != $id) {
       ...
       FRITZBOX_Log $hash, 5, "dectRing ID" . $id . " - \n" . FRITZBOX_Helper_Dumper($hash, $result, 5);
...
sub FRITZBOX_Get($@)
{
   ...
   if ($hash->{WEBCONNECT}) {
     ...
     # get Fritzbox luaQuery telcfg:settings/AlarmClock/list(Name,Active,Time,Number,Weekdays)
       ...
       my $result = FRITZBOX_call_Lua_Query( $hash, $val[0], "", "luaCall", $methode);
       ...
       my $result = FRITZBOX_call_LuaData($hash, "fnget", \@webCmdArray) ;
       ...
       if ($mode eq "pretty") {
         my $printJSON = FRITZBOX_Helper_encode_json($result);
...
#       my $tmp = FRITZBOX_Helper_Dumper($hash, $result);
       ...
       FRITZBOX_Log $hash, 3, "get $name $cmd " . $queryStr;
       ...
       if ($val[0] eq "igd") {
         $igd = 1;
         shift (@val);
         my $service_type = $val[1];
         my $para  = '<Envelope s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" xmlns:s="http://www.w3.org/2003/05/soap-envelope">'
                   . '<?xml version="1.0" encoding="utf-8"?>'
                   . '<s:Header></s:Header>'
                   .   '<s:Body>'
                   .   '<u:$service_command xmlns:u="urn:schemas-upnp-org:service:"' .$service_type. '>'
                   .   '</u:$service_command>'
                   .   '</s:Body>'
                   . '/s:Envelope';
         push(@val, "Content", $para);
       ...
       return FRITZBOX_get_TR064_ServiceList ($hash, undef, $val[0] . "desc.xml");
     ...
     $list .= " callApifromList:"          if defined $hash->{SECPORT} || $hash->{LUADATA} == 1 || $hash->{LUAQUERY} == 1;
...
sub FRITZBOX_SetGet_Proof_Params($@) {
   ...
   if ( int @val == 2 ) {
      if ($val[1] !~ /$mysearch/) {
         $mysearch =~ s/\^|\$//g;
         FRITZBOX_Log $hash, 2, "no valid $cmd parameter: " . $val[1] . " given.";
...
sub FRITZBOX_Readout_Start($)
{
   ...
   if (!defined $timerpara) {
     Log 1, "FATAL ERROR - FRITZBOX_Readout_Start: no parameter handed over";
     return "ERROR: FRITZBOX_Readout_Start: no parameter handed over";
   }
...
# Kill running process if "set update" is used
   if ( exists( $hash->{helper}{READOUT_RUNNING_PID} ) && $hash->{fhem}{readOutState} == 1 ) {
      ...
      FRITZBOX_Log $hash, 3, "Old readout process still running. Killing old process " . $hash->{helper}{READOUT_RUNNING_PID};
      BlockingKill( $hash->{helper}{READOUT_RUNNING_PID} );
...
# First run is an API check
   if ( $hash->{APICHECKED} =~ /[0|2]/ ) {
      ...
      readingsSingleUpdate( $hash, "state", "check APIs " . ($hash->{APICHECKED} ? "basic" : "full"), 1 );
      $runFn = "FRITZBOX_Readout_API_Check";
   } elsif ( $hash->{APICHECKED} < 0 ) {
      $interval = AttrVal( $name, "reConnectInterval", 180 ) < $hash->{CKECKAPI_TMOUT} ? $hash->{CKECKAPI_TMOUT} + 10 : AttrVal( $name, "reConnectInterval", 180 );
      $timeout  = $hash->{CKECKAPI_TMOUT};
...
# Dect device list
   ...
   if ( defined $result->{handsetCount} && $result->{handsetCount} =~ /[1-9]/ ) {
     ...
     foreach ( @{ $result->{dectUser} } ) {
       ...
       if ($intern) {
         unless ($noDect) {
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "dect".$runNo,                           $name;
#           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "dect".$runNo."_ID",                     $id;
...
#-------------------------------------------------------------------------------------
# Power Rate
   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_powerRate", $result->{box_powerRate} if $hash->{fhem}{fwVersion} > 0 && $hash->{fhem}{fwVersion} < 705;

#-------------------------------------------------------------------------------------
# Box Features
   if ($hash->{fhem}{fwVersion} < 750) {
     if ($result->{box_is_double_wlan}) {
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->cnt", 2;
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->names", "wlan2.4 wlan5";
     } else {
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->cnt", 1;
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->names", "wlan2.4";
     }
   }
...
# Box model, firmware and uptimes
   ...
   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_over_tls",           $result->{box_DNS_over_TLS}, "onoff";
   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "->DNSoTLS",                  $result->{box_DNS_over_TLS}, "onoff";

#   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv4_1st_DNS",          $result->{box_ipv4_1st_DNS};
#   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv4_2nd_DNS",          $result->{box_ipv4_2nd_DNS};
#   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv4_DHCP_LAN_DNS",     $result->{box_ipv4_DHCP_LAN_DNS};

   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv6",                   $result->{box_ipv6_Enabled}, "onoff";
   FRITZBOX_Readout_Add_Reading $hash, $roReadings, "->IPv6",                     $result->{box_ipv6_Enabled};

   if($result->{box_ipv6_Enabled}) {
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv6_Extern",          $result->{box_ipv6_Extern};
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv6_Prefix",          $result->{box_ipv6_Prefix};
     ...
     if ($hash->{LuaQueryCmd}{box_ipv6_Valid}{active}) {
       ...
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv6_Valid", ($mesh ne "slave") ? $str . " sec = " . $Tag . "T $Std:$Min:$Sek" : "";
     }

     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv6_Uptime",       $result->{box_ipv6_Uptime};     # in date/time

     if ($hash->{fhem}{fwVersion} < 731 && AttrVal($name, "enableBoxReadings", "") =~ /box_dns/) {
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv1_used_IPv6",      $result->{box_ipv6_1st_DNS};
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv2_used_IPv6",      $result->{box_ipv6_2nd_DNS};
     }
   }
...
sub FRITZBOX_Readout_Run_Web_LuaData($$$$)
{
   ...
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $resultData->{data}->{vars}, 5) if $resultData->{data}->{vars};
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $resultData->{data}->{rep_data}, 5) if $resultData->{data}->{rep_data};
   ...
   if ( $enableBoxReading =~ /box_notify/ && ($hash->{fhem}{fwVersion} >= 700) ) {
     ...
     if ($hash->{fhem}{fwVersion} >= 800) {
       ...
       if ($hash->{fhem}{fwVersion} < 810) {
         $result = FRITZBOX_call_javaScript($hash, "boxnotifications");
       } else {
         $result = FRITZBOX_call_javaScript($hash, "dino/boxnotifications");
       }
...
#     }
   ...
   if ($hash->{fhem}{fwVersion} >= 750) {
     FRITZBOX_Log $hash, 4, "wlanChannels - start getting data";
     ...
     if ($nbViews > 0) {
       ...
       eval {
         for(my $i = 0; $i <= $nbViews - 1; $i++) {
           ...
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, $bandName . "_active", $resultData->{data}->{$bandOrg}->{active}, "onoff";
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, $bandName . "_useable", $resultData->{data}->{$bandOrg}->{useable}, "yesno";
         }
       };

       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_wlanBand_cnt", $nbViews;
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->cnt", $nbViews;

       chop ($wNames);
       $wNames =~ s/GHz//g;
       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "fhem->multiple_wlan->names", $wNames;
     }

     FRITZBOX_Log $hash, 4, "wlanChannels - end getting data";
   }
   ...
   if ( AttrVal( $name, "enableWLANneighbors", "0") ) {
     ...
     FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $resultData->{data}->{scanlist}, 5);
   ...
   if ( $enableBoxReading =~ /box_led/ && ($hash->{fhem}{fwVersion} >= 700) ) {
     ...
     FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $resultData->{data}->{ledSettings}, 5);
         ...
         if ($nbViews > 0) {
           ...
           for(my $i = 0; $i <= $nbViews - 1; $i++) {
             ...
             if ( $resultData->{data}->{drain}->[$i]->{name} =~ /Gesamtsystem/) {
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_Rate_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_mainCPU_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_WLAN_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_DSL_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_FON_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_pwr_USB_24avg", $resultData->{data}->{drain}->[$i]->{cumPerc};
             }
         ...
         if ($SmartHome =~ /all|device/ ) {
           ...
           if ($nbViews > 0) {
             ...
             for(my $i = 0; $i <= $nbViews - 1; $i++) {
               ...
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_uid",             $resultData->{data}->{devices}->[$i]->{uid};
               ...
               if ( $resultData->{data}->{devices}->[$i]->{units} ) {
                 ...
                 for my $unit (0 .. scalar @{$units} - 1) {
                   if( $units->[$unit]->{'type'} eq 'THERMOSTAT' ) {
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_buttonLocked",   $units->[$unit]->{interactionControls}->[0]->{isLocked};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_externalLocked", $units->[$unit]->{interactionControls}->[1]->{isLocked};

                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_mode",       $units->[$unit]->{skills}->[0]->{mode};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_targetTemp", $units->[$unit]->{skills}->[0]->{targetTemp};
                     ...
                     if ($units->[$unit]->{skills}->[0]->{nextChange}->{timeSetting}) {
                       ...
                       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_modeNextChangeTime", $chgTime;
                     }

                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_adaptivHeatingActive",    $units->[$unit]->{skills}->[0]->{adaptivHeating}->{active};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_adaptivHeatingEnabled",   $units->[$unit]->{skills}->[0]->{adaptivHeating}->{isEnabled};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_adaptivHeatingSupported", $units->[$unit]->{skills}->[0]->{adaptivHeating}->{supported};

                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_temperatureDropMinutes",  $units->[$unit]->{skills}->[0]->{temperatureDropDetection}->{doNotHeatOffsetInMinutes};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_temperatureDropSens",     $units->[$unit]->{skills}->[0]->{temperatureDropDetection}->{sensitivity};

                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_timeControl",  $units->[$unit]->{skills}->[0]->{timeControl}->{isEnabled};

                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentState", $units->[$unit]->{skills}->[0]->{state}->{current};
                     if ($units->[$unit]->{skills}->[0]->{state}->{current} ne "DEFAULT") {

                       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentStateAction", $units->[$unit]->{skills}->[0]->{state}->{timeControl}->{description}->{action};
                       ...
                       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentStateEndTime", $endTime;
                     } else {
                       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentStateAction", "n/a";
                       FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentStateEndTime", "n/a";
                     }
                     ...
                     if ($units->[$unit]->{skills}->[0]->{timeControl}->{timeSchedules}) {
                       ...
                       for my $ts (0 .. scalar @{$allTimeSchedules} - 1) {
                         if( $allTimeSchedules->[$ts]{'name'} eq 'SUMMER_TIME' ) {
                           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_summerTimeAction", $allTimeSchedules->[$ts]{'actions'}->[0]->{description}->{action};
                           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_summerTimeEnabled", ( ( $allTimeSchedules->[$ts]{'actions'}->[0]->{'isEnabled'} ) ? 1 : 0 );
                           ...
                           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_summerTimePeriod", $stPeriod;
                           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_summerTimeRepetition", $allTimeSchedules->[$ts]{'actions'}->[0]->{timeSetting}->{repetition};
                     ...
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_assignedGroup", $units->[$unit]->{assignedGroup}->{displayName} if $units->[$unit]->{assignedGroup};
                     ...
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_powerPerHour", $units->[$unit]->{skills}->[0]->{powerPerHour};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_currentInAmp", $units->[$unit]->{skills}->[0]->{electricCurrentInAmpere};
                     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "shdevice" . $id . "_powerInWatt",  $units->[$unit]->{skills}->[0]->{powerConsumptionInWatt};
           ...
           my $XMLresult = FRITZBOX_call_Lua_Query( $hash, "webservices/homeautoswitch.lua?switchcmd=getdevicelistinfos", "", "luaCall") ;

           my $XMLtmp = FRITZBOX_Helper_analyse_Lua_Result($hash, $XMLresult);

           FRITZBOX_Log $hash, 4, "SmartHome - start getting XML data: $SmartHome";
           my $outhash;
           ...
           FRITZBOX_Log $hash, 4, "SmartHome - end getting XML data: $SmartHome";
         }
   ...
   #-------------------------------------------------------------------------------------
     ...
     if ( ($hash->{fhem}{fwVersion} >= 731) ) {
       ...
       FRITZBOX_Log $hash, 4, "DNS Info - start getting data";
       ...
       if($getDNSInfo) {
         ...
         if ( defined $resultData->{data}->{vars}->{ipv4}->{userdns}->{value} ) {
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv_user_IPv4", $resultData->{data}->{vars}->{ipv4}->{userdns}->{value}, "onoff";
           if ($resultData->{data}->{vars}->{ipv4}->{userdns}->{value} == 1) {
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv1_user_IPv4", $resultData->{data}->{vars}->{ipv4}->{firstdns}->{value};
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv2_user_IPv4", $resultData->{data}->{vars}->{ipv4}->{seconddns}->{value};
           }
         }

         if ( defined $resultData->{data}->{vars}->{ipv6}->{userdns}->{value} ) {
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv_user_IPv6", $resultData->{data}->{vars}->{ipv6}->{userdns}->{value}, "onoff";
           if ($resultData->{data}->{vars}->{ipv4}->{userdns}->{value} == 1) {
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv1_user_IPv6", $resultData->{data}->{vars}->{ipv6}->{firstdns}->{value};
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_dns_Srv2_user_IPv6", $resultData->{data}->{vars}->{ipv6}->{seconddns}->{value};
           }
         }
       }
         ...
         eval {
           for( $i = 0; $i <= $nbViews - 1; $i++) {
             ...
             if (defined $resultData->{data}->{connections}->[$i]->{ipv4}->{ip}) {
               FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv4_Extern", $resultData->{data}->{connections}->[$i]->{ipv4}->{ip};
             }
       ...
       FRITZBOX_Log $hash, 4, "DNS Info - end getting data";
     } else {

       FRITZBOX_Log $hash, 4, "wrong Fritz!OS for active DNS servers: $hash->{fhem}{fwVersionStr}" if $enableBoxReading =~ /box_dns/;
     ...
     if ($logFilter =~ /box_fon_Log/) {
       ...
       FRITZBOX_Log $hash, 4, "LOG_FON - start getting data: box_fon_Log";
       ...
       FRITZBOX_Log $hash, 4, "LOG_FON - end getting data: box_fon_Log";
     }
     ...
     if ( $hash->{fhem}{fwVersion} >= 721 ) {
       ...
       if ( $enableBoxReading =~ /box_globalFilter/ ) {
         ...
         FRITZBOX_Log $hash, 4, "globalFilter - start getting data: box_globalFilter";
         ...
         if ( defined($resultData->{data}->{filterList}) ) {
           FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $resultData->{data}->{filterList}, 5);
           ...
           FRITZBOX_Log $hash, 4, "globalFilter - end getting data: box_globalFilter";
         }
       ...
       if ($enableBoxReading =~ /box_energyMode/) {
         ...
         FRITZBOX_Log $hash, 4, "energyStatus - start getting data: box_energyMode";
         ...
         FRITZBOX_Log $hash, 4, "energyStatus - end getting data: box_energyMode";

       } #  end getting energy status

     } else {
       FRITZBOX_Log $hash, 4, "globalFilter/energyStatus not available for $hash->{fhem}{fwVersion}";
     }
     ...
     if ( AttrVal($name, "enableMobileInfo", 0) ) {
...
#                                                                   if $resultData->{data}->{sipNumberCount};
         ...
         FRITZBOX_Log $hash, 4, "wrong Fritz!OS for usb mobile: $hash->{fhem}{fwVersionStr}";
       }
     ...
     if ( AttrVal($name, "enableDocsisInfo", 0) && lc($avmModel) =~ "6[4,5,6][3,6,9][0,1]" ) {
...
#         my $resultData = FRITZBOX_Helper_process_JSON($hash, $TestSIS, "ee9c7ab49ebf6e09", "");
         ...
         FRITZBOX_Log $hash, 4, "wrong Fritz!OS: $hash->{fhem}{fwVersionStr} for docsis informations.";
       }

     } else {

       FRITZBOX_Log $hash, 4, "wrong AVM-Model: $avmModel for docsis informations.";
     ...
     if ($cpuInfo != 0 && ($hash->{fhem}{fwVersion} >= 800) ) {
       FRITZBOX_Log $hash, 4, "CPU-Info - start getting data";

       if ($hash->{fhem}{fwVersion} < 810) {
         $resultData = FRITZBOX_call_javaScript($hash, "cpu");
       } else {
         $resultData = FRITZBOX_call_javaScript($hash, "generic/cpu");
       }

       if (defined($resultData) && !defined($resultData->{Error}) ) {

         if ( defined($resultData->{data}) ) {
           FRITZBOX_Log $hash, 5, "CPU-Info - process data\n" . FRITZBOX_Helper_Dumper($hash, $resultData, 5);
...
#            $intValLeft = 10 * $cpuInfo * (-1);
           ...
           if( defined($resultData->{data}{StatCPU})) {
             ...
             FRITZBOX_Log $hash, 4, "CPU-Info - Comma: " . $cntComma;
             # $intValLeft = $cntComma * (-1) if ($cntComma * (-1) > $intValLeft);
             $intValRight = $cntComma if ($cntComma < $intValRight);
           } else {
             FRITZBOX_Log $hash, 3, "CPU-Info - StatCPU: not defined";
           }
...
#           my $StatRAMPhysFree     = join(",", reverse ((split /,/, $resultData->{data}{StatRAMPhysFree})[$intValLeft..$intValRight]));
           ...
           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpuCurrentInterval_Data", $resultData->{data}{StatCurrentInterval}
                                                                                 if $resultData->{data}{StatCurrentInterval};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpu_Data",                join(",", reverse ((split /,/, $resultData->{data}{StatCPU})[$intValLeft..$intValRight]))
                                                                                 if $resultData->{data}{StatCPU};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpuTemp_Data",            join(",", reverse ((split /,/, $resultData->{data}{StatTemperature})[$intValLeft..$intValRight]))
                                                                                 if $resultData->{data}{StatTemperature};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpuRAMCacheUsed_Data",    join(",", reverse ((split /,/, $resultData->{data}{StatRAMCacheUsed})[$intValLeft..$intValRight]))
                                                                                 if $resultData->{data}{StatRAMCacheUsed};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpuRAMStrictlyUsed_Data", join(",", reverse ((split /,/, $resultData->{data}{StatRAMStrictlyUsed})[$intValLeft..$intValRight]))
                                                                                 if $resultData->{data}{StatRAMStrictlyUsed};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_cpuRAMPhysFree_Data",     join(",", reverse ((split /,/, $resultData->{data}{StatRAMPhysFree})[$intValLeft..$intValRight]))
                                                                                 if $resultData->{data}{StatRAMPhysFree};

         }
       } else {
         FRITZBOX_Log $hash, 4, "CPU-Info - process data - " . $resultData->{Error} if defined $resultData->{Error};
         FRITZBOX_Log $hash, 2, "CPU-Info - process data - No data received" if !defined $resultData->{data};
       }
       FRITZBOX_Log $hash, 4, "CPU-Info - end getting data";
     }
   } # end for Model == "Box"
...
sub FRITZBOX_Readout_Run_Web_TR064($$$$)
{
   ...
   if ($avmModel =~ "Box") {
     ...
     if ($hash->{fhem}{fwVersion} >= 750) {
       ...
       FRITZBOX_Log $hash, 4, "2FA Info/State - start getting TR064 data";
       ...
       if ($tr064Result[0]->{UPnPError}) {
         $strCurl = FRITZBOX_Helper_Dumper($hash, \@tr064Result);
         FRITZBOX_Log $hash, 2, "2FA GetInfo -> \n" . $strCurl;
       } else {

         FRITZBOX_Log $hash, 5, "2FA GetInfo -> \n" . FRITZBOX_Helper_Dumper($hash, \@tr064Result, 5);

         if ($tr064Result[0]->{GetInfoResponse}) {
           if (defined $tr064Result[0]->{GetInfoResponse}->{NewEnabled}) {
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_2FAInfo", $tr064Result[0]->{GetInfoResponse}->{NewEnabled}, "onoff";
           }
         }
       }
       ...
       if ($tr064Result[0]->{UPnPError}) {
         $strCurl = FRITZBOX_Helper_Dumper($hash, \@tr064Result);
         FRITZBOX_Log $hash, 2, "2FA GetState -> \n" . $strCurl;
       } else {

         FRITZBOX_Log $hash, 5, "2FA GetState -> \n" . FRITZBOX_Helper_Dumper($hash, \@tr064Result, 5);

         if ($tr064Result[0]->{GetStateResponse}) {
           if (defined $tr064Result[0]->{GetStateResponse}->{NewState}) {
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_2FAState", $tr064Result[0]->{GetStateResponse}->{NewState};
           }
         }
       }
       FRITZBOX_Log $hash, 4, "2FA Info/State - end getting TR064 data";

     } else {

       FRITZBOX_Log $hash, 4, "wrong Fritz!OS: $hash->{fhem}{fwVersionStr} for 2FA information via TR064 or not a Fritz!Box";
     ...
     if (AttrVal($name, "enableMobileInfo", 0)) {  # FB mit Mobile Modem-Stick
       if ($hash->{fhem}{fwVersion} >= 750) {
       ...
       FRITZBOX_Log $hash, 4, "wrong Fritz!OS: $hash->{fhem}{fwVersionStr} for usb mobile via TR064 or not a Fritz!Box";
     ...
     for (my $i = 0; $i < @conn; $i++) {
       ...
       FRITZBOX_Log $hash, 4, "GetInfo for " . $wanConnection;

       @tr064CmdArray = (["WAN" .uc($conn[$i]). "Connection:1", $wanConnection, "GetInfo"]);

       @tr064Result = FRITZBOX_call_TR064_Cmd( $hash, 0, \@tr064CmdArray );

       if ($tr064Result[0]->{UPnPError}) {
         $strCurl = FRITZBOX_Helper_Dumper($hash, \@tr064Result);
         FRITZBOX_Log $hash, 4, "$wanConnection GetInfo -> \n" . $strCurl;
       } else {

         FRITZBOX_Log $hash, 5, "$wanConnection GetInfo -> \n" . FRITZBOX_Helper_Dumper($hash, \@tr064Result, 5);

         if ($tr064Result[0]->{GetInfoResponse}) {

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_name",               $tr064Result[0]->{GetInfoResponse}->{NewName}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewName};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_IPv4_Extern",        $tr064Result[0]->{GetInfoResponse}->{NewExternalIPAddress}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewExternalIPAddress};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_connection_Type",    $tr064Result[0]->{GetInfoResponse}->{NewConnectionType}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewConnectionType};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_connect",            $tr064Result[0]->{GetInfoResponse}->{NewConnectionStatus}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewConnectionStatus};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_last_connect_err",   $tr064Result[0]->{GetInfoResponse}->{NewLastConnectionError}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewLastConnectionError};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_last_auth_err",      $tr064Result[0]->{GetInfoResponse}->{NewLastAuthErrorInfo}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewLastAuthErrorInfo};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_mac_Address",        $tr064Result[0]->{GetInfoResponse}->{NewMACAddress}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewMACAddress};

           FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_connection_Trigger", $tr064Result[0]->{GetInfoResponse}->{NewConnectionTrigger}
                                                                                if $tr064Result[0]->{GetInfoResponse}->{NewConnectionTrigger};

           if ($tr064Result[0]->{GetInfoResponse}->{NewUptime}) {
             ...
             FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_" .$conn[$i]. "_uptimeConnect", $strCurl . " sec = " . $Tag . "T $Std:$Min:$Sek";
           }

           FRITZBOX_Log $hash, 4, "$wanConnection - end getting TR064 data";
         } else {
           FRITZBOX_Log $hash, 3, "$wanConnection - no GetInfoResponse getting TR064 data";
         }
       }
     }
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_IPv4_Extern", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_connection_Type", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_connect", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_last_connect_err", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_last_auth_err", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_mac_Address", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_connection_Trigger", "";
     FRITZBOX_Readout_Add_Reading $hash, $roReadings, "box_uptimeConnect", "";

     # end getting WANIPConnection/WANPPPConnection Info

     #-------------------------------------------------------------------------------------
     # TR064 with xml enveloap via SOAP request wanipconnection1
     # will be done for FritzBox 6[4,5,6][3,6,9][0,1] or if request for other models returns an error ($getInfo2cd)
     # UPNP hast to be enabled in FritzBox web frontend
     # $hash->{UPNP} == 100
     ...
     if ( $soap_activ ) {
       ...
       FRITZBOX_Log $hash, 4, "uptimeConnect- start getting TR064 WANIPConn1 data";

       $soap_resp = FRITZBOX_SOAP_Request($hash,$control_url . "WANIPConn1", $service_type, $service_command);

       if(defined $soap_resp->{Error}) {
         FRITZBOX_Log $hash, 2, "SOAP-ERROR -> " . $soap_resp->{Error};
       ...
       FRITZBOX_Log $hash, 4, "uptimeConnect- end getting TR064 WANIPConn1 data";

       # end box_uptimeConnect

       #-------------------------------------------------------------------------------------
       # box_ipExtern

       FRITZBOX_Log $hash, 4, "ipExtern- start getting TR064 WANIPConn1 data";
       ...
       FRITZBOX_Log $hash, 4, "ipExtern- end getting TR064 WANIPConn1 data";

       #-------------------------------------------------------------------------------------
       # box_ipv6 ->  NewPreferedLifetime, NewExternalIPv6Address, NewValidLifetime, NewPrefixLength
         
       FRITZBOX_Log $hash, 4, "ipv6 - start getting TR064 WANIPConn1 data";
       ...
       if(exists $soap_resp->{Error}) {
         FRITZBOX_Log $hash, 2, "SOAP/TR064-ERROR -> " . $soap_resp->{Error};
       ...
       FRITZBOX_Log $hash, 4, "ipv6 - end getting TR064 WANIPConn1 data";

       #-------------------------------------------------------------------------------------
       # box_ipv6_Prefix ->  NewPreferedLifetime, NewIPv6Prefix, NewValidLifetime, NewPrefixLength

       FRITZBOX_Log $hash, 4, "ipv6_Prefix - start getting TR064 WANIPConn1 data";
       ...
       FRITZBOX_Log $hash, 4, "ipv6_Prefix - end getting TR064 WANIPConn1 data";
     ...
     } # TR064 with xml enveloap via SOAP request
     ...
     FRITZBOX_Log $hash, 2, "unknown AVM Model $avmModel";
   }
...
sub FRITZBOX_Readout_Response($$$@)
{
  ...
  if ( defined $result->{sid} && !defined $result->{AuthorizationRequired}) {
    ...
    FRITZBOX_Log $hash, 4, "undefined situation\n" . " - " . $sub . "(" . $xline . ")"; # . FRITZBOX_Helper_Dumper($hash, $result, 4);
    push @{$roReadings}, "->WEBCONNECT", 0;
...
sub FRITZBOX_Readout_Done($)
{
  ...
  {
    Log 1, "Fatal Error - FRITZBOX_Readout_Done: no parameter handed over";
    return;
  }
...
sub FRITZBOX_Readout_Process($$)
{
 ...
 # Fatal Error: no hash parameter handed over
   unless (defined $hash) {
      Log 1, "Fatal Error - FRITZBOX_Readout_Process: no hash parameter handed over";
      return;
   }
...
#         FRITZBOX_Log $hash, 3, "2nd Level: $rName1 $rName2 - " . $rValue if $name eq "FritzBox";
       ...
       if ($rName2 eq "APICHECKED") {
         readingsBulkUpdate( $hash, "state", "check API done" ) ;
     ...
     if ($rName !~ /-<|->|box_fwUpdate|box_oem|readoutTime|Error/) {
       ...
       if ($rValue ne "" && $rName !~ /$reading_list/) {
         if ( $rFilter =~ /$filter_list/) {
           ...
           FRITZBOX_Log $hash, 4, "Reading ." . $rName . " = '$rValue'";
         } else {
           $rFilter = "." . $rName;
...
# Preparing SET lockFilterProfile -> result in retStat_lockFilterProfile 
      ...
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FRITZBOX_Set_lock_Filter_Profile";
   }
...
# Preparing SET DECTRING -> result in retStat_ring
      ...
      shift @val;
      $timeout = 20 + $val[1];
      FRITZBOX_Log $hash, 4, "timeout ring: $timeout";
      $cmdBufferTimeout = time() + $timeout;
      $handover = $name . "|" . join( "|", @val );
      $cmdFunction = "FRITZBOX_Set_dectRing_Phone";
   }
...
sub FRITZBOX_Readout_SetGet_Done($)
{
   ...
   unless (defined $string)
   {
      Log 1, "FATAL ERROR - FRITZBOX_Readout_SetGet_Done: no parameter handed over";
      return;
   }
...
# Check for remote APIs
   ...
   # Check if tr064 specification exists and determine TR064-Port
      ...
      if ($response->is_success) { #determine TR064-Port
         ...
         $tr064Port = FRITZBOX_init_TR064 ( $hash, $host );
         if ($tr064Port) {
   ...
   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "fhem->fwVersion", $osVersion;
   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "fhem->fwVersionStr", substr($fwV[1],0,2) . "." . substr($fwV[2],0,2);

   if ($apiError =~ /303|500/) {

     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->APICHECKED", -1;
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->CKECKAPI_TMOUT", $hash->{CKECKAPI_MAX_TMOUT};
     ...
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->APICHECKED", 1;
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "->CKECKAPI_TMOUT", 55;
...
#               push @roReadings, "RT_05_" . sprintf( "%02d", $lfnr++) . "_TR064_Services_" . $key, sprintf( "%.2f", time() - $startTime) . " : " . $stat;
         ...
         if($luaQueryOk) {
           ...
           if ( !defined $response->{sid} || defined $response->{Error} || defined $response->{AuthorizationRequired} ) {
             ...
             if ($osVersion >= 800) {
               FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "LuaQueryCmd->box_cpuTemp->active", 0;
...
} #end FRITZBOX_Readout_API_Check
...
sub FRITZBOX_Set_block_Incoming_Phone_Call($)
{
   ...
   if($val[0] =~ /new|tmp/) {
     ...
     my $search = FRITZBOX_Helper_Dumper($hash, $result);
...
sub FRITZBOX_Set_change_Profile($)
{
   ...
   if ($ProfileOK eq "false") {
     ...
     FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $lanDevice_Info, 5);
...
sub FRITZBOX_Set_enable_VPNshare_OnOff($)
{
   ...
   if ($vpnok == 0){
     ...
     FRITZBOX_Log $hash, 5, "$vpnState->{vpn_info} \n" . FRITZBOX_Helper_Dumper($hash, $vpnState, 5);
...
sub FRITZBOX_Set_lock_Filter_Profile($) {
   ...
   my $analyse = FRITZBOX_Helper_analyse_Lua_Result($hash, $resultData);
   if ( $analyse =~ /ERROR/) {
     FRITZBOX_Log $hash, 2, "lockFilterProfile - $analyse";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: $analyse";
     return FRITZBOX_Readout_Response($hash, $result, \@roReadings, 2);
   }
...
#  };
   ...
   if ($profileID eq "") {
     FRITZBOX_Log $hash, 2, "lockFilterProfile - profile name not found: $profileName";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: not found: $profileName";
     return FRITZBOX_Readout_Response($hash, $result, \@roReadings, 2);
   }
   ...
   if (defined $resultData->{data}) {
     ...
     if ($resultData->{data}{profileStatus} eq "limited") {
       FRITZBOX_Log $hash, 2, "lockfilterprofile - timetable is aktiv. Status and/or BPMJ could not be changed";
       FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: timetable is activ";
       return FRITZBOX_Readout_Response($hash, $result, \@roReadings, 2);
     }
     ...
     FRITZBOX_Log $hash, 2, "lockfilterprofile - unexpected result: " . $analyse;
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: unexpected result";
     return FRITZBOX_Readout_Response($hash, $result, \@roReadings, 2);
   }
   ...
   FRITZBOX_Log $name, 4, "data.lua: " . join(" ", @webCmdArray);

   $resultData = FRITZBOX_call_LuaData($hash, "data", \@webCmdArray) ;

   $analyse = FRITZBOX_Helper_analyse_Lua_Result($hash, $resultData);
   if ( $analyse =~ /ERROR/) {
     FRITZBOX_Log $hash, 2, "lockfilterprofile - error: " . $analyse;
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: $analyse";
     return FRITZBOX_Readout_Response($hash, $resultData, \@roReadings, 2);
   }

   if (defined $resultData->{data}{apply} && $resultData->{data}{apply} ne "ok") {
     FRITZBOX_Log $hash, 2, "lockfilterprofile - during apply - " . $resultData->{data}{apply};
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: $resultData->{data}{apply}"; 
     return FRITZBOX_Readout_Response($hash, $resultData, \@roReadings, 2);
   }

   if (!defined $resultData->{data}{apply}) {
     FRITZBOX_Log $hash, 2, "lockfilterprofile - apply could not be verified";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . "->ERROR: apply could not be verified";
     return FRITZBOX_Readout_Response($hash, $resultData, \@roReadings, 2);
   }

   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_lockFilterProfile", $val[0] . " set profile: $profileStatus BJMP: $bpjmStatus";

   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "readoutTime", sprintf( "%.2f", time()-$startTime);
   return FRITZBOX_Readout_Response($hash, $resultData, \@roReadings, 2, $sidNew);

} # end FRITZBOX_Set_lock_Filter_Profile
...
#  { FRITZBOX_Helper_Dumper($hash, FRITZBOX_call_javaScript($defs{FritzBox}, "landevice"))}

   if ($hash->{fhem}{fwVersion} < 810) {
     $result = FRITZBOX_call_javaScript($hash, "landevice");
   } else {
     $result = FRITZBOX_call_javaScript($hash, "generic/landevice");
   }
   ...
   if ($val[1] eq "off") {  
     ...
     if ($user) {
       ...
       if ($hash->{fhem}{fwVersion} < 810) {
         ...
         $result = FRITZBOX_write_javaScript($hash, "generic/landevice/landevice/" . $val[0], '{"device_class_user":"' . $dc_user . '","friendly_name":"' . $friendlyName . '","rrd":"0"}', "put");
       }
       # Abbruch wenn Fehler beim Lesen der Fritzbox-Antwort
       if ( defined $result->{Error} || defined $result->{AuthorizationRequired}) {
       ...
       if ($hash->{fhem}{fwVersion} < 810) {
         ...
         $result = FRITZBOX_write_javaScript($hash, "generic/user/user/" . $user, '{"filter_profile_UID":"' . $fp_uid . '","disallowed":"0"}', "put");
       }
       # Abbruch wenn Fehler beim Lesen der Fritzbox-Antwort
       if ( defined $result->{Error} || defined $result->{AuthorizationRequired}) {
...
#    { FRITZBOX_Helper_Dumper($hash, (FRITZBOX_call_javaScript($defs{FritzBox}, "trafficprio"))}

     FRITZBOX_Log $hash, 4, "locklandevice: no last change user defined for changing. Trying for traffic prio.";

     my $trafficPrio;
     if ($hash->{fhem}{fwVersion} < 810) {
       $trafficPrio = FRITZBOX_call_javaScript($hash, "trafficprio");
     } else {
       $trafficPrio = FRITZBOX_call_javaScript($hash, "generic/trafficprio");
     }
       ...
       if ($trafficPrio->{data}->{user}->[$j]->{mac} eq $mac) {
         ...
         if ($user) {
           ...
           if ($hash->{fhem}{fwVersion} < 810) {
             ...
             FRITZBOX_write_javaScript($hash, "generic/trafficprio/user/" . $user, "", "delete");
           }
     ...
     if ($user_ctrl) {
       ...
       if ($hash->{fhem}{fwVersion} < 810) {
         ...
         FRITZBOX_write_javaScript($defs{FritzBox},"generic/user/user/" . $user_ctrl, '{"disallowed":"1"}', "put");
       }
     } else {
       if ($hash->{fhem}{fwVersion} < 810) {
         FRITZBOX_write_javaScript($defs{FritzBox},"user/user", '{"filter_profile_UID":"' . $fp_uid . '","landeviceUID":"' . $val[0] . '","disallowed":"1","type":"1"}', "post");
       } else {
         FRITZBOX_write_javaScript($defs{FritzBox},"generic/user/user", '{"filter_profile_UID":"' . $fp_uid . '","landeviceUID":"' . $val[0] . '","disallowed":"1","type":"1"}', "post");
       }
     }
     # Abbruch wenn Fehler beim Lesen der Fritzbox-Antwort
     if ( defined $result->{Error} || defined $result->{AuthorizationRequired}) {
     ...
     if ($hash->{fhem}{fwVersion} < 810) {
       ...
       FRITZBOX_write_javaScript($defs{FritzBox},"generic/trafficprio/user", '{"ip":"' . $ip . '", "mac":"' . $mac . '","type":"1"}', "post");
     }
...
sub FRITZBOX_Set_lock_Landevice_OnOffRt($)
{
   ...
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $lock_res, 5);
...
sub FRITZBOX_Set_call_Phone($)
{
   ...
   unless ($hash->{SECPORT}) {
     FRITZBOX_Log $hash, 2, "set call: TR064 SECPORT not available.";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "set call: TR064 SECPORT not available.";
   ...
   @tr064Result = FRITZBOX_call_TR064_Cmd( $hash, 0, \@tr064CallArray);
   if ($tr064Result[0]->{Error}) {
     FRITZBOX_Log $hash, 2, "(set call): " . $tr064Result[0]->{Error};
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "(set call): " . $tr064Result[0]->{Error};
   ...
   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "call done: " . $extNo . " duration: " . $duration;
     ...
     if( $tr064Result[0]->{Error}) {
       ...
       if ( defined $result->{Error} || defined $result->{AuthorizationRequired}) {
         ...
         if ($hash->{fhem}{fwVersion} < 750) {
           FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "box_wlanBand_2.4GHz", $result->{box_wlan_24GHz}, "onoff";
           FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "box_wlanBand_5GHz", $result->{box_wlan_5GHz}, "onoff";
           FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "box_wlanBand_cnt", ($result->{box_wlan_5GHz} ? 2 : 1);
         }
         ...
         if ($nbViews > 0) {
           ...
           eval {
             for(my $i = 0; $i <= $nbViews - 1; $i++) {
               ...
               FRITZBOX_Readout_Add_Reading $hash, \@roReadings, $bandName . "_active", $result->{data}->{$bandOrg}->{active}, "onoff";
               FRITZBOX_Readout_Add_Reading $hash, \@roReadings, $bandName . "_useable", $result->{data}->{$bandOrg}->{useable}, "yesno";
...
sub FRITZBOX_Set_dectRing_Phone($)
{
   ...
   $result = FRITZBOX_call_Lua_Query( $hash, "fon_devices/edit_dect_ring_tone.lua?idx=$val[0]&startringtest=1&ringtone=$val[2]&xhr=1&useajax=1", "", "luaCall", "post");

   if ( defined $result->{Error} ) {

     my $msg = "set $cmd: Lua_Query error verifiying $cmd: $val[0]";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", $msg;
     FRITZBOX_Log $hash, 2, $msg . " - " . $result->{Error};

   } else {

     $result = FRITZBOX_call_Lua_Query( $hash, "fon_devices/edit_dect_ring_tone.lua?idx=$val[0]&startringtest=2&xhr=1&useajax=1", "", "luaCall", "post");
     ...
     if ( defined $result->{Error} ) {
       ...
       $result = FRITZBOX_call_Lua_Query( $hash, "fon_devices/edit_dect_ring_tone.lua?idx=$val[0]&stopringtest=1&ringtone=$val[2]&xhr=1&useajax=1", "", "luaCall", "post");
       ...
       if ( defined $result->{Error} ) {
         ...
         FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "dectRing done: ID_" . $val[0] . " duration: " . $val[1];
       }
     }
   }
...
} # end FRITZBOX_Set_newRing_Phone
...
sub FRITZBOX_Set_ring_Phone($)
{
   ...
   unless ($hash->{SECPORT}) {
     FRITZBOX_Log $hash, 2, "set ring: TR064 SECPORT not available.";
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "set ring: TR064 SECPORT not available.";
...
# Check ClickToDial
   ...
   if ($tr064Result[0]->{Error}) {
     FRITZBOX_Log $hash, 2, "(set ring phonePort): " . $tr064Result[0]->{Error};
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "phonePort: " . $tr064Result[0]->{Error};
     return $name."|2|Error (set ring phonePort): ".$tr064Result[0]->{Error};
   }
   ...
   if ($portName) {
     ...
     @tr064Result = FRITZBOX_call_TR064_Cmd( $hash, 0, \@tr064CmdArray );
     FRITZBOX_Log $hash, 4, "Switch ClickToDial on, set ring port '$portName'";
   }

   $intNo =~ s/,/#/g;

   FRITZBOX_Log $hash, 4, "Ringing $intNo for $duration seconds";
   FRITZBOX_Log $hash, 5, "X_VoIP:1 x_voip X_AVM-DE_DialNumber NewX_AVM-DE_PhoneNumber **" . $intNo . "#";
   ...
   if ($tr064Result[0]->{Error}) {
     FRITZBOX_Log $hash, 2, "(set ring): " . $tr064Result[0]->{Error};
     FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "ring: " . $tr064Result[0]->{Error};
     return $name."|2|Error (set ring): " . $tr064Result[0]->{Error};
   }
   ...
   unless ( $duration == 0 || $duration == -1 ) {
      ...
      @tr064Result = FRITZBOX_call_TR064_Cmd( $hash, 0, \@tr064CmdArray ) if $hash->{SECPORT};
      if ($tr064Result[0]->{Error}) {
        FRITZBOX_Log $hash, 2, "(set ring hangUp): " . $tr064Result[0]->{Error};
        FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "hangUp: " . $tr064Result[0]->{Error};
        return $name."|2|Error (set ring): " . $tr064Result[0]->{Error};
      }
   }

   FRITZBOX_Readout_Add_Reading $hash, \@roReadings, "retStat_ring", "ring done: **" . $intNo . " duration: " . $duration;
...
sub FRITZBOX_Get_WLAN_globalFilters($) {
   ...
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $result->{data}->{filterList}, 5);
...
#  border(8),cellspacing(10),cellpadding(20)
   ...
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $result->{data}->{init}->{boxConnections}, 5);
...
sub FRITZBOX_Get_WLAN_Environment($) {
   ...
   FRITZBOX_Log $hash, 5, "\n" . FRITZBOX_Helper_Dumper($hash, $result->{data}->{scanlist}, 5);
...
############################################
# Test: { FRITZBOX_Helper_Dumper($defs{"FritzBox"}, FRITZBOX_Get_SmartHome_Devices_List($defs{"FritzBox"}, "17", "test"))}
...
#
# xhr 1 lang de page sh_dev xhrId all
# { FRITZBOX_Helper_Dumper($defs{"FritzBox"},FRITZBOX_Get_SmartHome_Devices_List($defs{"FritzBox"}, "17"))}
# { FRITZBOX_Helper_Dumper($defs{"FritzBox"},FRITZBOX_Get_SmartHome_Devices_List($defs{"FritzBox"}, "18"))}
...
sub FRITZBOX_Get_SmartHome_Devices_List($@) {
   ...
   FRITZBOX_Log $hash, 3, "FRITZBOX_SmartHome_Device_List (Fritz!OS: $hash->{fhem}{fwVersionStr}) ";
   ...
   if ($devID && $command =~ /read|save|test/) {
     ...
     FRITZBOX_Log $hash, 3, "Processing SmartHome_Device_List: read|save|test";
     ...
     for my $i (0 .. @{$devData} - 1) {
       ...
       if( $devData->[$i]{'id'} eq $devID ) {
         FRITZBOX_Log $hash, 3, "Processing SmartHome_Device_List: $devData->[$i]{'id'}";
         ...
         FRITZBOX_Log $hash, 5, "getting smart home info -> " . FRITZBOX_Helper_Dumper($hash, $devData->[$i], 5);
         ...
         if (!$isSocket) {
           ...
           if($hash->{fhem}{fwVersion} >= 820 && $ret{device_name_category} eq "THERMOSTAT") {
             ...
             FRITZBOX_Log $hash, 5, "getting smart home info -> " . FRITZBOX_Helper_Dumper($hash, $result1, 5);
             ...
             FRITZBOX_Log $hash, 5, "SmartHome Device info HKR\n" . FRITZBOX_Helper_Dumper($hash, \@sortRet, 5) if keys(%ret);
         ...
         for my $j (0 .. scalar @{$unitData} - 1) {
           ...
           # Mikrofon in Smart-Steckdose
             ...
             if ($skills->{'isEnabled'}) {
               ...
               FRITZBOX_Log $hash, 4, "allTimeSchedules:\n" . FRITZBOX_Helper_Dumper($hash, $allTimeSchedules, 4);

               for my $ts (0 .. scalar @{$allTimeSchedules} - 1) {
                 Log3 $hash, 3, "kind[$ts]: " . $allTimeSchedules->[$ts]{'kind'};
             ...
             for my $ts (0 .. scalar @{$allTimeSchedules} - 1) {
               ...
               if( $allTimeSchedules->[$ts]{'kind'} eq 'WEEKLY_TIMETABLE') {
                 ...
                 if (!$allTimeSchedules->[$ts]{'isEnabled'}) {
                   ...
                   # xhr 1 lang de page sh_dev xhrId all
                   # { FRITZBOX_Helper_Dumper($hash, FRITZBOX_Get_SmartHome_Devices_List($defs{"FritzBox"}, "31", "test"))}
     ...
     if( keys(%ret) ) {
       ...
       FRITZBOX_Log $hash, 5, "SmartHome Device info\n" . FRITZBOX_Helper_Dumper($hash, \%ret, 5) if keys(%ret);
...
sub FRITZBOX_Get_User_Info_List($) {
   ...
   eval {
      FRITZBOX_Log $hash, 5, "evaluating user info: \n" . FRITZBOX_Helper_Dumper($hash, $result->{user_info}, 5);
   };
...
EOD
   ...
   if (ref($resp_init->decoded_content) eq "HASH") {
     FRITZBOX_Log $hash, 4, "XML_RESONSE:\n" . FRITZBOX_Helper_Dumper($hash, $resp_init->decoded_content);
     %retHash = ( "Info" => "SOAP response: " . $resp_init->status_line, "Response" => FRITZBOX_Helper_Dumper($hash, $resp_init->decoded_content) ) ;
   ...
   foreach( @{$cmdArray} ) {
      ...
      if ($@) {
        ...
        unless( $res ) { # Transport-Error
          ...
          @{$cmdArray} = ();
          # my $fdetail = FRITZBOX_Helper_Dumper($hash, $res->faultdetail); # returns value of 'detail' element as string or object
          # return "Error\n".$fdetail;
...
# Security Port anfordern
   ...
   FRITZBOX_Log $hash, 5, "SecPort-String " . FRITZBOX_Helper_Dumper($hash, $s, 5);

   my $port = $s->result;
   FRITZBOX_Log $hash, 5, "SecPort-Result " . FRITZBOX_Helper_Dumper($hash, $s->result, 5);
...
sub FRITZBOX_call_Lua_Query($$@)
{
   ...
   if ( $f_lua eq "luaQuery") {
     ...
     FRITZBOX_Log $hash, 4, "luaFunction call: " . $url;
   ...
   if ($methode eq "get") {
     ...
     FRITZBOX_Log $hash, 4, "post -> URL: $url with Content: $chgStr";
   ...
   FRITZBOX_Log $hash, 4, "Response: " . $response->content . "\n" . $response->content;
...
sub FRITZBOX_call_LuaData($$$@)
{
   ...
   FRITZBOX_Log $hash, 4, "Response(status_line): " . $response->status_line . "\nResponse(content)\n" . $response->content;
   ...
   if ( ($data =~ m/\<form id="uiMainForm" class="narrow" name="main_form" method="POST" action="\/fon_devices\/fondevices_list.lua"\>/igs) ) {
     ...
     if ( ($data =~ m/fonDevices: (.*?)fonNumbers:/gsm) ) {
       ...
       FRITZBOX_Log $hash, 4, "Response fonDevices: \n" . $fonDev;
   ...
   if ( ($data =~ m/\<form id="uiMainForm" method="POST" action="\/fon_devices\/edit_dect_ring_tone.lua.*?"\>(.*?)\<\/form\>/igs) ) {
     ...
     FRITZBOX_Log $hash, 4, "Response Data Ring Tone: \n" . $1;
     ...
     if ($toneCont =~ m/\<select size="1" id="uiViewRingTestTone" name="ring_test_tone" class="no-watch"\>(.*?)\<\/select\>/gsm) {
       ...
       FRITZBOX_Log $hash, 4, "ring Tones: \n" . $1;
   ...
   if ($queryStr =~ /home_auto_temp_view/) {
     FRITZBOX_Log $hash, 4, "Response Data - page home_auto_edit_view: \n" . $data;
   }
   ...
   if ($queryStr =~ /home_auto_hkr_edit/) {
     ...
     if ( $data =~ m/\<form name="mainform" method="POST" action="\/net\/home_auto_hkr_edit.lua"\>(.*?)\<div id="domReady" style="display:none"\>\<\/div\>/igs ) {
       ...
       ($tmp) = ($smartCont =~ m/\<input type="hidden" name="Roomtemp" value="(.*?)" id="uiNum:Roomtemp"\>/igs);
       $profile_content .= '"Roomtemp":"' . $tmp . '",' if $tmp;

       ($tmp) = ($smartCont =~ m/var cfg = \{"offset":.*?,"hkr_temp":.*?,"ExtTempsensorID":(\d+),"room_temp":\}/igs);
       $profile_content .= '"ExtTempsensorID":"' . ($tmp == 0 ? "tochoose" : $tmp) . '",' if $tmp;
...
#     $profile_content .= '}},"sid":"' . $result->{sid} . '"}';
   ...
   my $resultJSON = FRITZBOX_Helper_process_JSON($hash, $response->content, $result->{sid}, $charSet, $sidNew);
...
sub FRITZBOX_write_javaScript($$;@)
{
   ...
   FRITZBOX_Log $hash, 4, "_content: " . FRITZBOX_Helper_Dumper($hash, $data, 4);
...
#   if ($jsonText =~ /<html|"pid": "logout"|<head>/) {
   if ($jsonText =~ /\"pid\": \"logout\"/) {
      FRITZBOX_Log $hash, 3, "JSON: Old SID not valid anymore. ResetSID";
   ...
   FRITZBOX_Log $hash, 5, "JSON: " . FRITZBOX_Helper_Dumper($hash, $jsonResult, 5);
   ...
   if ($fromJS == 1) {
     ...
     if (ref ($jsonResult) eq "ARRAY") {
       FRITZBOX_Log $hash, 4, "JSON: is ARRAY";
       my %ReturnHash = (
           "sid"    => $sid,
           "sidNew" => $sidNew,
           "result" => $jsonResult
       );
       FRITZBOX_Log $hash, 5, "JSON =>result: " . FRITZBOX_Helper_Dumper($hash, \%ReturnHash, 5);
       return \%ReturnHash;

     } elsif (!defined $jsonResult->{data} && !defined $jsonResult->{result}) {
       FRITZBOX_Log $hash, 4, "JSON: no {data} or {result}";
       my %ReturnHash = (
           "sid"    => $sid,
           "sidNew" => $sidNew,
           "data"   => $jsonResult
       ...
       FRITZBOX_Log $hash, 5, "JSON =>data: " . FRITZBOX_Helper_Dumper($hash, $jsonResult, 5);
       return $jsonResult;
#       return \%ReturnHash;
     
     } else {
       FRITZBOX_Log $hash, 4, "JSON: Standard";
       $jsonResult->{sid}    = $sid;
       $jsonResult->{sidNew} = $sidNew;
       $jsonResult->{Error}  = $jsonResult->{error}  if defined $jsonResult->{error};
     ...
     if ( ref ($jsonResult) ne "HASH") {
        chop($jsonText);
        FRITZBOX_Log $hash, 4, "no HASH/ARRAY from JSON returned\n (" . $jsonText . ")";
        my %retHash = ("Error" => "no HASH/ARRAY from JSON returned", "ResetSID" => "1");
        return \%retHash;
     }
...
sub FRITZBOX_Helper_analyse_Lua_Result($$;@)
{
   ...
   if (ref ($result->{result}) eq "ARRAY" || ref ($result->{data}) eq "HASH" ){
     ...
     $tmp = FRITZBOX_Helper_Dumper($hash, $result);

     FRITZBOX_Log $name, 5, "result is array/hash: " . $tmp;
     ...
     FRITZBOX_Log $name, 5, "result is text: " . $tmp;
     ...
     if (ref ($result->{data}) eq "ARRAY" || ref ($result->{data}) eq "HASH" ) {
       $tmp .= "\n" . FRITZBOX_Helper_Dumper($hash, $result) if $retData == 1;
       FRITZBOX_Log $name, 5, "data is array/hash: " . $tmp;
     }
     elsif (defined $result->{data} ) {
       $tmp .= "\n" . $result->{data} if $retData == 1;
       FRITZBOX_Log $name, 5, "data is text: " . $tmp;
     }
   }
     ...
     $tmp = "Unexpected result: " . FRITZBOX_Helper_Dumper($hash, $result);
   }
...
sub FRITZBOX_Helper_read_Password($)
{
   ...
   if ($sub !~ /open_Web_Connection|call_TR064_Cmd|Readout_API_Check|Rename/) {
...
sub FRITZBOX_Helper_Dumper($$;@) {
   ...
   my $modVerbose = AttrVal($name,"verbose", AttrVal("global","verbose", 1));
   ...
   if (ref $reference) {
     if (ref($reference) eq '') {
       FRITZBOX_Log $hash, 3, "unblessed Reference in $sub.$xline";
       $retValue = $reference;
     } else {
       FRITZBOX_Log $hash, 5, "blessed Reference " .ref($reference). " in $sub.$xline";
       if($logLevel > $modVerbose) {
         $retValue = "tiny log level for Dumper: $logLevel";
         FRITZBOX_Log $hash, 5, "blessed Refrence in $sub.$xline - tiny log level for Dumper: $logLevel";
       } else {
...
#       } else {
#         $json_doc =~ s/\n//g;
#         $result = FRITZBOX_Helper_process_JSON($hash, $json_doc, "12345678", undef, 0);
...
<div>
   ...
   <ul>
      ...
      <li><a name="smartHome"></a>
         ...
         <br>
         <dt><code>set &lt;name&gt; smartHome &lt;deviceID&gt; &lt;boost:0..120&gt;</code></dt>
         <dd>sets the radiator thermostat to boost for 1 to 120 minutes. boost:0 deactivates boost mode</dd>
         <br>
   ...
   <ul>
      ...
      <li><a name="reConnectInterval"></a>
         ...
         <br>
         After network failure or FritzBox unavailability. Default is 180 (seconds). The smallest possible value is 85 (seconds).
      </li><br>
   ...
   <ul><br> 
      ...
      <li><b>mac_</b><i>nn_nn_nn_nn_nn_nn</i> - MAC address and name of an active network device.<br>
                                             If no MAC address is provided, e.g., switch or VPN, the FritzBox DeviceID is used instead of the MAC address.<br>
      ...
      <li><b>retStat_wlanGuestParams</b> - Return Status</li><br> 
...
<div>
   ...
   <ul>
      ...
      <li><a name="smartHome"></a>
         <dt><code>set &lt;name&gt; smartHome &lt;apply&gt; &lt;scenario:ID|template:ID&gt; or &lt;routine&gt; &lt;enable:ID | disable:ID&gt; or &lt;deviceID&gt; &lt;tempOffset:degrees | tmpAdjust:degrees | tmpPerm:0|1 | boost:minutes | switch:0|1 | automatic:0|1  | preDefSave:name | preDefDel:name| preDefLoad[:deviceID]:name[:A|:G]&gt;</code></dt>
         <ul>
         <dt><code>set &lt;name&gt; smartHome &lt;apply&gt; &lt;scenario:ID|template:ID&gt;</code></dt>
         ...
         <br>
         <dt><code>set &lt;name&gt; smartHome &lt;deviceID&gt; &lt;boost:0..120&gt;</code></dt>
         <dd>setzt den Heizköperregeler auf boost für 1..120 Minuten. boost:0 deaktiviert den Boost-Modus</dd>
         <br>
         <dt><code>set &lt;name&gt; smartHome &lt;deviceID&gt; &lt;switch:0|1&gt;</code></dt>
         <dd>schaltet den Steckdosenadapter aus|an.</dd>
         <br>
