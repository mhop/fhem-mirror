...
sub listInternalData {
  ...
  if ( $hash->{TYPE} eq 'AutomowerConnect' ) {
    ...
    $ret .= '<caption><b>Authentification Data</b></caption><tbody>'; 

    $ret .= '<tr class="column odd"><td> Authentification URL</td><td>' . AUTHURL . '</td></tr>';
    $ret .= '<tr class="column even"><td> Client-Id</td><td>' . $hash->{helper}{client_id} . '</td></tr>';
    $ret .= '<tr class="column odd"><td> Grant-Type</td><td>' . $hash->{helper}{grant_type} . '</td></tr>';
    $ret .= '<tr class="column even"><td> User-Id</td><td>' . ReadingsVal($name, '.user_id', '-') . '</td></tr>';
    $ret .= '<tr class="column odd"><td> Provider</td><td>' . ReadingsVal($name, '.provider', '-') . '</td></tr>';
    $ret .= '<tr class="column even"><td> Scope</td><td>' . ReadingsVal($name, '.scope', '-') . '</td></tr>';
    $ret .= '<tr class="column odd"><td> Token Type</td><td>' . ReadingsVal($name, '.token_type', '-') . '</td></tr>';
    $ret .= '<tr class="column even"><td> Token Expires</td><td> ' . FmtDateTime( ReadingsVal($name, '.expires', '0') ) . '</td></tr>';
    $ret .= '<tr class="column odd"><td> Access Token</td><td style="word-wrap:break-word; max-width:40em">' . ReadingsVal($name, '.access_token', '0') . '</td></tr>';
