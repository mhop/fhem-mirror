...
sub RecvPacket {
   ...
   #
   #	recv($self->{TCPClient}, $buf, $Size , &MSG_NOSIGNAL);
   ...
   #	}
	...
	if ( defined($buf) && length($buf) == $Size ) {
		...
		if ( defined($buf) ) {
			...
			main::Log3 undef, 3,
			  "TCPClient RecvPacket error (IP= " . $self->{Peer} . ").";
		}
		return $self->SetLastError( &errTCPConnectionReset, $buf );
	}

	#	if ( !defined($res) ) #todo fix error handling for RecvPacket
	#	{

	#		main::Log3 undef, 3,"TCPClient RecvPacket error.";
	#		# print "RecvPacket recv error, Size $Size, $buf \n";
	#		return $self->SetLastError(&errTCPConnectionReset,$buf);
	#	} elsif ($res != 0) {
	#		main::Log3 undef, 3,"TCPClient RecvPacket error : $res";
	#		return $self->SetLastError(&errTCPConnectionReset,$buf);
	#	}
	#	return ($self->SetLastError(0),$buf);
...
#        	Purge() if (shutdown($self->{TCPClient}, &SD_SEND)==0);#Anmerkung SD_SEND = 1
...
sub RecvISOPacket {
	...
	while ( ( $self->{LastError} == 0 ) && !$Done ) {
		...
		if ( $self->{LastError} == 0 ) {
			...
			main::Log3 undef, 5,
			  "TCPClient RecvISOPacket Expected Size = $Size";
			...
			if ( $Size == 7 ) {
				...
				if ( ( $Size > $maxlen ) || ( $Size < &MinPduSize ) ) {
					main::Log3 undef, 3,
					    "TCPClient RecvISOPacket PDU overflow (IP= "
					  . $self->{Peer}
					  . "): size = $Size , maxPDULength = "
					  . $self->{PDULength};
	...
	if ( $self->{LastError} == 0 ) {
		...
		if ( $main::attr{global}{verbose} <= 5 ) {
			...
			main::Log3 undef, 5,
			  "TCPClient RecvISOPacket (IP= " . $self->{Peer} . "): $b";
		}
...
#	my $Read;
#
#	if ($self->{LastError}!= WSAECONNRESET)
#    	{
#        	if (CanRead(0)) {
#           		do
#           		{
#               			recv($self->{TCPClient}, $Trash, 512, &MSG_NOSIGNAL );
#	           	} while( defined ($Trash) && length($Trash)==512);
#        	}
#    	}
#}
...
sub ReadArea () {
	...
	$WordSize = 2 if ( ( $Area == &S7AreaCT ) || ( $Area == &S7AreaTM ) );
	...
	while ( ( $TotElements > 0 ) && ( $self->{LastError} == 0 ) ) {
		...
		if ( $main::attr{global}{verbose} <= 5 ) {
			...
			main::Log3 undef, 5,
			  "TCPClient ReadArea (IP= " . $self->{Peer} . "): $b";
		}
					...
					if ( ( $myLength - 18 == $SizeRequested ) ) {
						...
						if ( $myPDU[31] == 0xFF ) {
							...
							main::Log3 undef, 3,
							  ...
							  . $self->{Peer}
							  . ") returned data not expected size: $b";
						}
						else {
							my $b = join(
								", ",
