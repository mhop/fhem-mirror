// $Id: fhem.js 5485 2014-04-08 10:10:56Z betateilchen $
...
var keywords = ["at","attr","define","delete","deleteattr","deletereading","displayattr","fhem","get","getstate",
...
CodeMirror.registerHelper("hint", "fhem", function hintfhem(cm, callback, options) {
	...
	// only device attribute names
	    ...
	    var device = RegExp.$2;
		word = RegExp.$3;
		var d = searchDevice(devices,device);
	...
	function searchDevice(items, name){
		var startIndex  = 0,
		...
		return (iname != name) ? null : items[middle];
	}
	
	function finish(keys,devs,vars,word) {
		var res = [];
		keys.foreach( function( k, v ) {
			if( v.lastIndexOf(word,0)==0)
		...
		vars.foreach( function( k, v ) {
			if( v.lastIndexOf(word,0)==0)
				res.push({text:v,className:"hintvariable"});
		});
		callback( {
			list: res,
