...
DbLog_ParseEvent($$)
{
  ...
  # FHT
	...
	$unit= "";
     ...
     if($reading =~ m(-temp)) { $value=~ s/ \(Celsius\)//; $unit= "Â°C"; }
     if($reading =~ m(temp-offset)) { $value=~ s/ \(Celsius\)//; $unit= "Â°C"; }
  ...
  # KS300
     ...
     if($reading eq "temperature") { $value=~ s/ \(Celsius\)//; $unit= "Â°C"; }
  ...
  # HMS
     ...
     if($event =~ m(T:.*)) { $reading= "data"; $value= $event; }
     if($reading eq "temperature") { $value=~ s/ \(Celsius\)//; $unit= "Â°C"; }
	 if($reading eq "temperature") { $value=~ s/([-\.\d]+).*/$1/; $unit= "Â°C"; } #OWTHERM
     if($reading eq "humidity") { $value=~ s/ \(\%\)//; $unit= "%"; }
