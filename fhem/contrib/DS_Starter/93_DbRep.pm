##########################################################################################################
# $Id: 93_DbRep.pm 17609 2018-10-23 20:27:51Z DS_Starter $
##########################################################################################################
#       93_DbRep.pm
#
...
our %DbRep_vNotesIntern = (
  "8.5.0"  => "27.10.2018  versioNotes revised, EXCLUDE of reading/device possible (DbRep_specsForSql changed) ",
...
sub DbRep_specsForSql($$$) {
 ...
 my ($idanz,$edanz)     = (0,0);
 if($device =~ /EXCLUDE=/) {
     ($idevice,$edevice) = split(/EXCLUDE=/,$device);
 ...
 if($reading =~ /EXCLUDE=/) {
     ($ireading,$ereading) = split(/EXCLUDE=/,$reading);
