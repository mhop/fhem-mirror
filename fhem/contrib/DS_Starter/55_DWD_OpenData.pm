...
sub Define {
  ...
  my $nextTime = gettimeofday() + int(rand(480));
  ::readingsSingleUpdate($hash, 'nextcycle', ::FmtTime($nextTime), 1);
  $hash->{'.firststart'} = 1;
  ::InternalTimer($nextTime, 'DWD_OpenData::Timer', $hash, 0);
...
sub Timer {
  my ($hash) = @_;
  my $name = $hash->{NAME};
  my $fst  = delete $hash->{'.firststart'} // 0;
  ...
  my $qofprec = ::AttrVal($name, 'forecastDataPrecision', 'low') eq 'low' ? 0 : 2;
  
  # reschedule next run for max 600 seconds past next quarter
  ::RemoveInternalTimer($hash);
  my $nextTime = timegm(0, $actQuarter*15, $tHour, $tMday, $tMon, $tYear) + 900 + int(rand(600));
  ::readingsSingleUpdate($hash, 'nextcycle', ::FmtTime($nextTime), 1);
  ::InternalTimer($nextTime, 'DWD_OpenData::Timer', $hash);

  if ($fst || ($actQuarter == $qofprec && (defined($hash->{".fetchAlerts"}) && $hash->{".fetchAlerts"}))) {
...
sub GetForecastStart {
  ...
  if (::AttrVal($name, 'forecastDataPrecision', 'low') eq 'high') {
...
sub ProcessForecast {
  ...
  eval {
    ...
    if (!@properties) {                                                          # no selection: use defaults
      if (::AttrVal($name, 'forecastDataPrecision', 'low') eq 'high') {
    ...
    for my $xmlString (@xmlStrings) {                                               
      ...
      if ($placemarkNodeList->size()) {
        my $pos = getStationPos ($name, $station, $placemarkNodeList);

        ::Log3 $name, 5, "$name: ProcessForecast: use position >$pos< of station >$station<";       
         
        my $placemarkNode = $placemarkNodeList->get_node($pos);
...
#  determine XML position depending on Attr forecastDataPrecision
sub getStationPos {
  my $name              = shift;
  my $station           = shift;
  ...
  if (::AttrVal($name, 'forecastDataPrecision', 'low') eq 'low') {
    return $pos;
  }
  
  my $ls = $placemarkNodeList->size();
